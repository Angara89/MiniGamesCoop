...
class Field(DrawnObj):
	...
	def __init__(self, tblock: Block):
		...
		self.rectField = pygame.Rect(self.indent, self.indent, self.myBlock.size[0] - (2 * self.indent), self.myBlock.size[1] - (2 * self.indent))
		...
		self.mainRect = pygame.Rect(rectInnerX, rectInnerY, (self.quantityCellsX * self.PIXEL_CELL), (self.quantityCellsY * self.PIXEL_CELL))
		pygame.draw.rect(self.myBlock, (100, 100, 100), self.mainRect)
		
		
	def get_new_anchorPoint_on_coordin(self, coord: Coord):
		return (self.mainRect.left + (self.PIXEL_CELL * coord.x), self.mainRect.top + (self.PIXEL_CELL * coord.y))
		
	def draw_DrawnObj_by_coord(self, drawnObj: DrawnObj, coord:()):
		pass
	
	def add_new_DrawObj(self, drawnObj):
		super().add_new_DrawObj(drawnObj)
